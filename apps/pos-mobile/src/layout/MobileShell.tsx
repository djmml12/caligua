...
export default function MobileShell({ onLogout }: MobileShellProps) {
  ...
  /* ── Cobrar por sub-cuentas (dividir) ────── */
  async function handleSplitAllPaid(saleId: number | null) {
    const cartTotalSnap   = ticket.cartTotal;
    const orderIdToCancel = ticket.currentOrderId;
    ticket.clearAfterPay();
    if (orderIdToCancel != null) {
      await orders.cancelOrder(orderIdToCancel);
    } else {
      void refreshOrders();
    }
      ...
      <main className="ms-content">
        ...
        {activeTab === "ticket" && (
          <TicketTab
            ...
            onSplitAllPaid={handleSplitAllPaid}

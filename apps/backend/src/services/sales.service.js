 ...
 *  Advisory lock serializes concurrent callers so two transactions
 *  never read the same MAX and collide on idx_sales_monthly_num. */
const computeNextMonthlyNumber = async (qFn) => {
  // 7891011 = arbitrary stable key; held for duration of the transaction
  await qFn(`SELECT pg_advisory_xact_lock(7891011)`, []);
...
export async function createSaleService(userId, reference = null) {
  return withTransaction(async (client) => {
    ...
    return {
      ...(newId ? await selectSaleHeaderById(newId, client) : {}),
      id: newId,
...
export async function createAndPaySaleService(userId, {
  ...
  return withTransaction(async (client) => {
    ...
    // Idempotencia: evita doble cobro si el cajero reintenta tras un timeout
    if (requestId) {
      const dupRows = getRows(await queryFn(
        `SELECT id, total, tip_amount FROM sales WHERE client_request_id = ?`,
        [requestId]
      ));
      if (dupRows[0]) {
        return { id: dupRows[0].id, total: Number(dupRows[0].total), tip_amount: Number(dupRows[0].tip_amount ?? 0), duplicate: true };
      }
    }
    ...
    await queryFn(
      `UPDATE sales
          SET total             = ?,
              ...
              client_request_id = ?
        WHERE id = ?`,
      [total, userId, finalTipAmount, finalTipPercentage, requestId, saleId]
    );

...
import {
  ...
  buildEscPosReservationBytes,
} from "../utils/escpos-builder.js";
import { getReservationByIdService } from "../services/reservations.service.js";
...
/** POST /print/kitchen-ticket  —  print kitchen ticket for open order
 *  Body: { saleId, targets?: ('kitchen'|'bar')[] }
...
export const printKitchenTicket = async (req, res) => {
  try {
    ...
    if (!saleId) {
      return res.status(400).json({ success: false, message: "saleId requerido" });
    }
...
/** POST /print/reservation  —  print a reservation ticket */
export const printReservation = async (req, res) => {
  try {
    const { reservation_id } = req.body ?? {};
    if (!reservation_id) {
      return res.status(400).json({ success: false, message: "reservation_id requerido" });
    }
    ...
    const reservation = await getReservationByIdService(Number(reservation_id));
    if (!reservation) {
      return res.status(404).json({ success: false, message: "Reserva no encontrada" });
    }

    const bytes = await buildEscPosReservationBytes(reservation, config);
    const buf   = Buffer.from(bytes);
    ...
    res.json({ success: true, message: "Ticket de reserva enviado a la impresora" });
  } catch (error) {
    console.error("PRINT RESERVATION ERROR:", error);
    res.status(500).json({ success: false, message: error.message || "Error al imprimir" });
  }
};

...
const drawPdfTotalsGrid = (doc, totals, options = {}) => {
  ...
  totals.forEach((item, index) => {
    ...
    // Fondo con sombra (fillOpacity en lugar de hex de 8 dígitos que pdfkit no soporta)
    doc.save().fillOpacity(0.094).roundedRect(x + 2, y + 2, cardWidth, cardHeight, 8).fill("#000000").restore();
...
const drawSectionTitle = (doc, title, options = {}) => {
  ...
  doc.save().fillOpacity(0.133).rect(m, top, w, 20).fill(T.RED).restore();
  doc

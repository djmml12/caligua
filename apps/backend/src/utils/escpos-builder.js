...
export const buildEscPosReservationBytes = async (reservation, printerConfig = {}) => {
  const cfg = resolveConfig(printerConfig);
  const { contentWidth, bottomMm } = cfg;
  const bytes = [];
  ...
  bytes.push(...centerAlign(), ...boldOn());
  pushLine(bytes, "*** RESERVA ***");
  bytes.push(...boldOff(), ...leftAlign());
  pushLine(bytes, divider(contentWidth));

  pushLine(bytes, pairLine("Cliente:", sanitizeText(reservation.customer_name || ""), contentWidth));
  if (reservation.phone) {
    pushLine(bytes, pairLine("Telefono:", sanitizeText(reservation.phone), contentWidth));
  }
  pushLine(bytes, pairLine("Fecha:",   fmtDate(reservation.date), contentWidth));
  if (reservation.time) {
    pushLine(bytes, pairLine("Hora:", sanitizeText(String(reservation.time).slice(0, 5)), contentWidth));
  }
  pushLine(bytes, divider(contentWidth));
  ...
  pushLine(bytes, divider(contentWidth));
  bytes.push(...boldOn());
  pushLine(bytes, pairLine("TOTAL:", fmtMoney(reservation.total || 0), contentWidth));
  bytes.push(...boldOff());

  if (reservation.notes) {
    pushLine(bytes, divider(contentWidth));
    wrapText(`Nota: ${reservation.notes}`, contentWidth).forEach((line) => pushLine(bytes, line));
  }
